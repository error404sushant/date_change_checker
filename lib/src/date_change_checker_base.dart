...
class DateChangeChecker {
    ...
    try {
      if (Platform.isIOS) {
        ...
        throw PlatformException(
          ...
          message:
              'Platform not supported. Only iOS and Android are supported.',
        );
      }
  ...
  static Future<AutoDateTimeStatus> _checkAutoDateTimeStatusIOS({
    ...
    try {
      developer.log('iOS: Checking auto date/time status using NTP comparison',
          name: 'DateChangeChecker');

      // Get device time in UTC
      final DateTime deviceTimeUtc = DateTime.now().toUtc();
      developer.log('iOS: Device time (UTC): $deviceTimeUtc',
          name: 'DateChangeChecker');
      ...
      developer.log('iOS: NTP time (UTC): $ntpTimeUtc',
          name: 'DateChangeChecker');

      // Calculate time difference in milliseconds
      final int timeDifferenceMs =
          deviceTimeUtc.difference(ntpTimeUtc).inMilliseconds.abs();
      developer.log(
          'iOS: Time difference: ${timeDifferenceMs}ms (threshold: ${thresholdMs}ms)',
          name: 'DateChangeChecker');
      ...
      developer.log('iOS: Failed to check auto date/time status via NTP: $e',
          name: 'DateChangeChecker', level: 1000);

      // Fallback: If NTP fails, we cannot determine the status reliably
      // Return OFF as a conservative approach
      developer.log(
          'iOS: Falling back to AUTO_DATE_TIME_OFF due to NTP failure',
          name: 'DateChangeChecker',
          level: 900);
  ...
  /// Throws [PlatformException] if platform is not supported or if an error occurs
  static Future<DateTimeChangeResult>
      detectComprehensiveDateTimeChange() async {
    try {
      ...
      throw PlatformException(
        code: e.code,
        message:
            'Failed to detect comprehensive date/time changes: ${e.message}',
        details: e.details,
      );
    }
  }
  ...
  static Future<DateTime> fetchNtpTime({
    ...
    try {
      developer.log('Fetching NTP time from server: $ntpServer',
          name: 'DateChangeChecker');
      ...
      developer.log('Successfully fetched NTP time: $ntpTime',
          name: 'DateChangeChecker');
      return ntpTime;
    } catch (e) {
      developer.log('Failed to fetch NTP time: $e',
          name: 'DateChangeChecker', level: 1000);
  ...
  static Future<TimeSyncResult> detectTimeSyncIssues({
    ...
    try {
      ...
      final int timeDifferenceMs =
          deviceTime.difference(ntpTime).inMilliseconds.abs();
      ...
      developer.log('Time synchronization check failed: $e',
          name: 'DateChangeChecker', level: 1000);
      rethrow;
    }
  }
  ...
  static Future<bool> detectTimeModification({
    ...
    try {
      ...
      developer.log('Time modification detection failed: $e',
          name: 'DateChangeChecker', level: 1000);
      rethrow;
    }
  }
  ...
  /// Throws [PlatformException] if platform is not supported or if an error occurs
  @Deprecated(
      'Use isDateTimeChanged() instead. Will be removed in a future version.')
  ...
  static Future<ComprehensiveTimeAnalysis> performComprehensiveTimeAnalysis({
    ...
    try {
      developer.log('Starting comprehensive time analysis',
          name: 'DateChangeChecker');
      ...
      developer.log('Comprehensive time analysis completed',
          name: 'DateChangeChecker');
      return analysis;
    } catch (e) {
      developer.log('Comprehensive time analysis failed: $e',
          name: 'DateChangeChecker', level: 1000);
      rethrow;
    }
  }
...
class ComprehensiveTimeAnalysis {
  ...
  bool get isAutoDateTimeEnabled =>
      autoDateTimeStatus == AutoDateTimeStatus.AUTO_DATE_TIME_ON;
...
class DateTimeChangeResult {
  ...
  factory DateTimeChangeResult.fromMap(Map<String, dynamic> map) {
    return DateTimeChangeResult(
      changeType:
          DateTimeChangeType.fromString(map['changeType'] ?? 'noChange'),
